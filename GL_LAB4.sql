...
INSERT INTO ORDERS VALUES(111,1000,"2021-09-10",4,5);
...
SELECT COUNT(*), CUS_GENDER FROM CUSTOMER c JOIN ORDERS o ON 
	c.CUS_ID = o.CUS_ID WHERE O.ORD_AMOUNT >= 3000 GROUP BY CUS_GENDER;
...
SELECT C.CAT_ID ,C.CAT_NAME, T2.PRO_NAME, MIN(SUPP_PRICE) FROM CATEGORY C 
	JOIN (SELECT * FROM PRODUCT P 
    JOIN (SELECT PRO_ID AS ID, MIN(SUPP_PRICE) AS SUPP_PRICE FROM SUPPLIER_PRICING
    GROUP BY ID HAVING MIN(SUPP_PRICE)) AS T1 
    ON P.PRO_iD = T1.ID) AS T2 
    ON C.CAT_ID = T2.CAT_ID
    GROUP BY C.CAT_ID HAVING MIN(T2.SUPP_PRICE);
 /*  Display the Id and Name of the Product ordered after “2021-10-05” */   
SELECT * FROM PRODUCT P 
	JOIN (SELECT PRO_ID FROM SUPPLIER_PRICING SP 
    JOIN ORDERS O 
	ON SP.PRICING_ID = O.PRICING_ID WHERE O.ORD_DATE > "2021-10-05") AS T2
    ON P.PRO_ID = T2.PRO_ID;
    
 /* Display the Id and Name of the Product ordered after “2021-10-05”*/   
SELECT * FROM CUSTOMER 
	WHERE LOWER(CUS_NAME) LIKE "a%" 
    OR LOWER(CUS_NAME) LIKE "%a"
